		...
		<div className={styles.itemInfo}>
			...
			<div className={styles.infoBox}>
				...
				<div className={styles.bottomSide}>
					...
					<div className={styles.logo}>
						...
						<div className={styles.text}>Черная пятница</div>
					</div>
			...
			<div className={styles.priceBox}>
				<div className={styles.priceWithCard}>
					<div
						className={styles.number}>{getPriceWithWhitespace(getPriceWithCard(getPriceWithDiscount(item.price, item.discount)), item.amount)} ₽
					</div>
				...
				<div className={styles.price}>
					<span
						className={styles.priceWithDiscount}>{getPriceWithWhitespace(getPriceWithDiscount(item.price, item.discount), item.amount)} ₽</span>
				...
				<div className={styles.bonus}>
					...
					<span>{getBonus(getPriceWithDiscount(item.price, item.discount), item.amount)} {item.seller.title}</span>
				</div>
			</div>
		</div>
		<div className={styles.amountBox}>
			<AmountCounter amount={item.amount} remains={item.remains} />
			{item.amount > 1 &&
				<div
					className={styles.oneItemPrice}>
					{getPriceWithWhitespace(getPriceWithCard(getPriceWithDiscount(item.price, item.discount)))} ₽
					/ шт.
				</div>}
		</div>
	</div>
